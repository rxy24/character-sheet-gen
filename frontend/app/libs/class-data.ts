...
export async function fetchClass({ className }: { className: String }) {
    const cookieStore = await cookies()
    const token = cookieStore.get(TOKEN_LABEL)?.value
    const method = 'GET'
    const url: string =  `${process.env.API_URL!}/classes?className=${encodeURIComponent(className.toString())}`
...
export async function fetchAllClass(){
    ...
    const token = cookieStore.get(TOKEN_LABEL)?.value
    const method = 'GET'
    const url: string = process.env.API_URL! + '/classes'
